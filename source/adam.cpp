...
private:
    ...
    struct cell_t
    {
        ...
        priority_t priority() const {
            assert(specifier_m == access_interface_input || specifier_m == access_interface_output
    ...
    any_regular_t calculate_indexed(const line_position_t& position, 
                                    const array_t& expression, std::size_t index) {
...
void sheet_t::implementation_t::touch(const name_t* first, const name_t* last)
{
    ...
    while (first != last)
    {
        ...
        */
        ...
        if (iter != input_index_m.end()) index.push_back(*iter);
        ...
        const array_t& conditional, const relation_t* first, const relation_t* last)
{
    ...
    for (vector<name_t>::iterator f = cell_set.begin(), l = cell_set.end(); f != l; ++f) {
        ...
        if (p == output_index_m.end() || !p->interface_input_m)
            throw stream_error_t(make_string("interface cell ", f->c_str(),
...
void sheet_t::implementation_t::flow(cell_bits_t& priority_accessed)
{
    ...
    for (relation_cell_set_t::iterator f(relation_cell_set_m.begin()), l(relation_cell_set_m.end());
            f != l; ++f) {
        if (!f->resolved_m) cells.insert(cells.end(), f->edges_m.begin(), f->edges_m.end());
    }
    sort(cells);
    ...
    */
    ...
    while(!cells.empty()) {
        cell_t& cell = *cells.back(); cells.pop_back();
        
        if (cell.relation_count_m == 0) continue;
        
        cell.resolved_m = true;
                
        for (relation_index_t::iterator f = cell.relation_index_m.begin(), l = cell.relation_index_m.end();
                f != l; ++f) {
            ...
            for (relation_set_t::iterator tf((*f)->terms_m.begin()), tl((*f)->terms_m.end());
                ...
                bool resolved = false;
                for(vector<name_t>::iterator fc = tf->name_set_m.begin(), lc = tf->name_set_m.end();
                        fc != lc; ++fc) {
                        
                    index_t::iterator iter = output_index_m.find(*fc);
                    assert(iter != output_index_m.end());
                
                    if (iter->resolved_m) { resolved = true; break; }
                }
            ...
            for (std::size_t n = 0, count = term->name_set_m.size(); n != count; ++n) {
                ...
                if (count == 1) {
                    cell.term_m = boost::bind(&implementation_t::calculate_expression,
                        boost::ref(*this), term->position_m, term->expression_m);
                } else {
                    cell.term_m = boost::bind(&implementation_t::calculate_indexed,
                        boost::ref(*this), term->position_m, term->expression_m, n);
                }
                ...
                if (cell.relation_count_m) cells.push_back(&cell);
                else cell.resolved_m = true;
            }
            ...
            for (relation_set_t::iterator tf((*f)->terms_m.begin()), tl((*f)->terms_m.end());
                ...
                remaining_cells.insert(remaining_cells.end(), tf->name_set_m.begin(), tf->name_set_m.end());
            }
            ...
            for(vector<name_t>::iterator fc = remaining_cells.begin(), lc = remaining_cells.end();
                ...
                if (iter->resolved_m) continue;                       
                --iter->relation_count_m;
            }
        }
        assert(cell.relation_count_m == 0 && "Cell still belongs to relation but all relations resolved.");
    }
}
            ...
            last_cell(relation_cell_set_m.end()); current_cell != last_cell; ++current_cell)
    {
        ...
        if (!calculate_expression(current_cell->position_m, current_cell->conditional_m).cast<bool>())
        {
            for (vector<cell_t*>::iterator f = current_cell->edges_m.begin(),
                    l = current_cell->edges_m.end(); f != l; ++f) {
                --(*f)->relation_count_m;
            }
...
// calculate the output/interface_output/invariant cells and apply.
     ...
     iter != last; ++iter)
    {
        ...
        if (!cell.evaluated_m) {
            ...
            if (cell.specifier_m == access_interface_output) get_stack_m.push_back(std::make_pair(cell.name_m, false));
...
// Then we can check the invariants -
             ...
             last (invariant_index_m.end()); iter != last; ++iter)
    {
        ...
        if (!cell.state_m.cast<bool>()) poison |= cell.contributing_m;
    }
...
any_regular_t sheet_t::implementation_t::get(name_t variable_name)
{
    ...
    */
    ...
    if (get_stack_m.size() && (get_stack_m.back().first == variable_name)) {
        assert(cell.interface_input_m && "FATAL (sparent) : Only interface cells should be on the get stack.");
    ...
    if (cell.specifier_m == access_interface_output) get_stack_m.push_back(std::make_pair(variable_name, false));
